...
**P0 (admin per-couple publish/undo, Finish Week) and P1a (Home activity + league points for a revealing week, Spoiler-Free mid-reveal Mark watched with un-mark, Pacific copy) are built; P1a's un-mark needs `supabase/apply-unmark-watched.sql` run in the Dashboard. Still to do: P1b (Standings, Score History, Couples Leaderboard, roster cards, Results pull from a revealing week) and P2 polish. Key decisions are in CLAUDE.md.** Original notes:  draft all scores after the East airing, then reveal couple scores one by one during the West feed, publishing final status (safe/eliminated) separately. Super admin only; viewers opt in via a per-viewer "Follow live" toggle on Home (off by default, Realtime push, Spoiler-Free viewers hidden until they opt in). Points update per reveal, so it needs a partial recompute (Dance Card judges' points per couple; survival, podium, Curtain Call and Grand Finale only at final-status publish), a split of `results_published_at` into scores-revealed vs results-published, and a per-dance reveal flag/RPC. Home's activity lines currently read only published `dance_scores`.
