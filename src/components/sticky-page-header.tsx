...
// Pins the top bar + switcher/carousel for the fan tabs. Must be
...
export function StickyPageHeader({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      ref={ref}
      className="sticky top-0 z-40 -mx-4 flex flex-col gap-3 border-b border-border bg-background px-4 pb-2 pt-[max(1rem,env(safe-area-inset-top))]"
    >
      {children}
    </div>
  );
