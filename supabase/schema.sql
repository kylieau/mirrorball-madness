...
-- Undo for the above: lowers the mark so a week (and every later one) is
-- hidden again. Only ever lowers, only touches the caller's own row.
create function public.unmark_episodes_watched_from(p_week_number int)
returns void
language plpgsql
security definer
set search_path = ''
