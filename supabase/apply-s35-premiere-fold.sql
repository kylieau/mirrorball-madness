...
--
-- Choices:
--   Theme on the folded premiere: 'Premiere' (or the non-null combined themes).
...
begin
  ...
  if v_night1.id is not null and v_night2.id is not null then
    ...
    if v_fold then
      ...
      set
        theme = case
          when coalesce(nullif(trim(v_night1.theme), ''), '') <> ''
           and coalesce(nullif(trim(v_night2.theme), ''), '') <> ''
           and lower(trim(v_night1.theme)) <> lower(trim(v_night2.theme))
            then trim(v_night1.theme) || ' / ' || trim(v_night2.theme)
          when coalesce(nullif(trim(v_night1.theme), ''), '') <> '' then trim(v_night1.theme)
          when coalesce(nullif(trim(v_night2.theme), ''), '') <> '' then trim(v_night2.theme)
          else 'Premiere'
        end,
