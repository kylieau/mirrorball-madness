...
- **Settings picker lists only fully published weeks** (plus the current mark, so its value always resolves), newest first; picking a lower week hides later results again. No queries on page load: data comes from a server action prefetched after paint.
