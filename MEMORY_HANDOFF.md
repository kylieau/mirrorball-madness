...
Docs-only park on `cursor/episode-dropdown-layout-backlog-7612` (draft PR #16 into `main`). No UI or code. Do not implement the dropdown move or sticky-nav change from this PR. Do not merge from the agent.
...
2. Do not implement the episode-dropdown move or sticky-nav change until those backlog items are picked up.
