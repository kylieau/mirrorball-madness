...
- Per selected completed episode: elim pick(s) vs actual (✓/✗), top-scorer pick vs highest summed `dance_scores.total_score` (shared `findTopScorerCoupleIds` with scoring), points from `weekly_manager_scores.prediction_points` (not recomputed). Hits collapse to a single **Nailed it** line (layout A); misses are one **strike → actual** line (layout 2).
